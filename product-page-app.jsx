        ...
        <div className="shell">
          ...
          <section className="hero">
            ...
            <aside className="details">
              <div><div className="eyebrow">{categoryLabel}</div><h1>{text(product.name, "ARMOR Product")}</h1><p className="lead">{lineBreaks(specText)}</p><div className="meta-row"><span className="meta-chip">{leaf}</span><span className="meta-chip">{text(product.manufacturer, "ARMOR")}</span><span className="meta-chip">{available ? "Published / Available" : "Published"}</span>{badge && <span className={"tag-pill " + (badge.toLowerCase().includes("hot") ? "hot" : "")}>{badge}</span>}</div>{swatches.length > 0 && <div className="color-row" aria-label="Product colors">{swatches.map((color) => <span className="detail-color" style={{ background: colorBackground(color) }} title={colorLabel(color)} key={color}></span>)}</div>}</div>
